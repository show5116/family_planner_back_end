...
- ✅ **빠른 기록 `POST /diaries/append`** (upsert, 행 잠금 + P2002 1회 재시도)
...
- ✅ 회고 카드 첨부 썸네일 + 다국어 라벨(`unit`·`amount`)
- ✅ 썸네일 업로드 경로 (클라이언트가 생성해 업로드, 영상·이미지 공통)
- ⬜ 영상 첨부 (Phase 3) — 서버 검증·썸네일 경로는 준비 완료, 앱 UI·압축이 남음
...
- ⬜ **프리미엄 등급 판매** — `family_planner_premium_monthly` 영수증이 `premium` tier로
  반영되는지, 스토어 웹훅(갱신·해지·환불)이 premium 상품 ID도 처리하는지 확인 필요.
  `ad_free`만 매핑돼 있으면 갱신이 반영되지 않는다. 다운그레이드 시 기존 파일은
  지우지 않고 신규 업로드만 차단하는 정책은 이미 구현돼 있다 ([21-diary.md](docs/features/21-diary.md))
...
| Phase 6: 최적화 및 배포 | 🟡 진행 중 | 50% (적립금 완료, 구독 스토어 검증·웹훅 완료, 루틴 배지/랭킹/알림까지 완료, 다이어리 Phase 1 완료) |
