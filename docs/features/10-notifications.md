...
| token     | String (Unique)   | FCM 디바이스 토큰 (VarChar 500)     |
| platform  | DevicePlatform    | 디바이스 플랫폼 (IOS, ANDROID, WEB) |
...
| enabled          | Boolean       | 알림 활성화 여부 (기본값: true)           |
| weatherAlertHour | Int           | 날씨 알림 발송 시각 (기본값: 7 = 오전 7시) |
| createdAt        | DateTime      | 생성일시                                  |
| updatedAt        | DateTime      | 수정일시                                  |
...
| sent      | Boolean       | FCM 발송 완료 여부 (기본값: false) |
...
| ------ | ----------------------------- | ----------------------- | ---------- |
| POST   | `/notifications/token`        | FCM 디바이스 토큰 등록  | JWT        |
| DELETE | `/notifications/token/:token` | FCM 디바이스 토큰 삭제  | JWT, Owner |

### 알림 설정
| Method | Endpoint                    | 설명                 | 권한 |
| ------ | --------------------------- | -------------------- | ---- |
| GET    | `/notifications/settings`   | 알림 설정 조회       | JWT  |
| PUT    | `/notifications/settings`   | 알림 설정 업데이트   | JWT  |

### 알림 히스토리
| Method | Endpoint                        | 설명                    | 권한       |
| ------ | ------------------------------- | ----------------------- | ---------- |
| GET    | `/notifications`                | 알림 목록 (페이지네이션) | JWT        |
| GET    | `/notifications/unread-count`   | 읽지 않은 알림 개수     | JWT        |
| PUT    | `/notifications/read-all`       | 전체 알림 읽음 처리     | JWT        |
| PUT    | `/notifications/:id/read`       | 알림 읽음 처리          | JWT, Owner |
| DELETE | `/notifications/:id`            | 알림 삭제               | JWT, Owner |

### 예약 / 어드민
| Method | Endpoint                   | 설명                         | 권한        |
| ------ | -------------------------- | ---------------------------- | ----------- |
| POST   | `/notifications/schedule`  | 예약 알림 등록               | JWT         |
| POST   | `/notifications/test`      | 테스트 알림 전송             | JWT, Admin  |
...
src/notification/
  dto/
    ...
    notification-response.dto.ts   — DeviceTokenDto, NotificationSettingDto, NotificationDto, PaginatedNotificationsDto, UnreadCountResponseDto, MarkAllAsReadResponseDto
    index.ts
  enums/
    notification-category.enum.ts  — NotificationCategory (SCHEDULE, TODO, HOUSEHOLD, SAVINGS, ASSET, CHILDCARE, GROUP, SYSTEM, WEATHER, FRIDGE)
    device-platform.enum.ts
    fcm-topic.enum.ts
  notification.controller.ts
  notification.service.ts
  notification-token.service.ts
