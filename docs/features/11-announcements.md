...
- 페이지 + 카테고리별 캐시 키 생성 (모든 사용자가 동일한 캐시 공유)
- 사용자별 읽음 상태(`isRead`)만 동적으로 추가
- 캐시 히트 시 DB 조회 완전 생략
...
**6. 스케줄러 동작** (최적화 완료 + Redis 네이티브 명령어)
- Cron 스케줄: 매 10분마다 실행
- **조회수 동기화** (Race Condition 완전 해결):
  - `INCR`로 원자적 카운트 증가 (조회 시)
  - `SADD`로 추적 Set 관리 (O(1) 성능)
  - `SMEMBERS`로 동기화 대상 조회
  - DB 업데이트 후 `DECRBY`로 카운트 차감 (Reset 대신 차감으로 데이터 유실 방지)
  - 카운트가 0 이하면 키 삭제 및 `SREM`으로 추적 Set에서 제거
  - **병렬 처리**: `Promise.all`로 모든 Redis GET 요청 병렬 처리 (N+1 문제 해결)
- **읽음 처리 동기화** (배치 처리 + 원자성 + 성능 최적화):
  - **메모리 보호**: BATCH_SIZE(1,000건)로 나누어 처리
  - **원자적 Pop 연산**: `SPOP`으로 조회와 동시에 Set에서 제거
  - **Race Condition 방지**: `SETNX`로 중복 읽음 처리 원자적 차단
  - **무한 루프**: 모든 데이터 처리될 때까지 배치 반복
  - `createMany` Bulk Insert로 단일 쿼리 처리 (Connection Storm 방지)
  - **불필요한 DB 조회 제거**: `skipDuplicates`만으로 중복 처리 (findMany 제거)
  - **병렬 처리**: `Promise.all`로 모든 Redis GET 요청 병렬 처리 (N+1 문제 해결)
- **키 존재 확인 최적화**: `EXISTS` 명령어 사용 (값 전체 조회 X)
- 실패한 항목은 로그 기록
...
- **N+1 쿼리 완전 해결**:
  - `_count`를 사용하여 읽음 수 조회 최적화 (관계 로드 제거)
  - 스케줄러에서 `Promise.all` 병렬 처리로 순차 await 제거
  - Redis GET 요청 1,000번 순차 → 병렬 처리로 **~1,000배 성능 향상**
- **Connection Storm 방지**: 배치당 1,000건 × 1번 Bulk Insert (무제한 → 제한적 처리)
- **메모리 보호**: 10만 건 데이터도 1,000건씩 나누어 안전하게 처리
- **Race Condition 완전 해결**:
  - `INCR` + `DECRBY`: 조회수 동기화 시 데이터 유실 방지 (Reset → 차감 방식)
  - `SETNX`: 읽음 처리 중복 원자적 방지
  - `SADD`: 추적 Set 관리 중복 자동 처리 (O(1))
  - `SPOP`: 배치 Pop 시 각 요청이 다른 아이템 획득
- **Big Key 문제 해결**: 배열 → Set 자료구조로 O(N) → O(1) 성능 개선
- **캐시 효율성 향상**: 사용자별 캐시가 아닌 공통 캐시로 메모리 사용량 ~99% 감소
- **Redis 원자성 보장**: Pop 방식으로 조회 중 서버 재시작 시에도 데이터 유실 방지
- **has 메서드 최적화**: `EXISTS` 명령어로 값 조회 없이 존재 확인 (네트워크 부하 감소)
- **불필요한 DB 조회 제거**: 읽음 처리 동기화 시 findMany 제거 (`skipDuplicates`만 사용)
- **응답 속도 향상**: Redis 캐시 히트 시 ~10ms 이내 응답
- **확장성 향상**: DB Connection Pool 고갈 방지 + 메모리 안정성 + 동시성 안전
