...
- **`GET /routines/stats/daily-streak`(신규)**: 일일 목표 기준 전체 연속 달성 스트릭. `currentStreakDays`/`longestStreakDays`/`totalAchievedDays`/`perfectWeeksCount`/`todayAchieved`/`todayCheckedCount`/`todayTargetCount`와, 목표 조정 제안용 `recent14Days`(최근 14일 `achievedDays`/`exceededDays`/`totalDays`/`averageCheckedCount`) 포함. **오늘이 아직 미달성이어도 스트릭을 끊지 않고 어제까지의 값을 유지**(자정이 지나야 끊김). 대상 습관이 0개였던 날은 스트릭을 끊지 않고 건너뜀. 스트릭 계산은 `RoutineSettingHistory` 최초 생성일부터 시작(그 이전은 아직 목표 개념이 없던 기간이라 감사 대상에서 제외) — `ALL` 모드만 써온 사용자도(한 번도 `COUNT`로 바꾼 적 없어도 `PATCH`를 호출한 적이 있다면) 동일하게 전체 스트릭 조회 가능. `RoutineSettingHistory`가 아예 없는 사용자(설정을 한 번도 변경한 적 없음)는 모든 값이 0/빈 값으로 응답(에러 아님). `totalAchievedDays`(목표 달성일 누적)/`perfectWeeksCount`(월~일 7일 전부 달성한 주의 수)는 `computeDailyGoalAchievementSummary`가 이미 계산하던 값을 그대로 내려주는 것으로 추가 쿼리가 없다 — 배지 판정 기준 3종(`GOAL_STREAK_DAYS`/`GOAL_TOTAL_DAYS`/`GOAL_PERFECT_WEEK`)의 현재값이 이 응답 하나로 전부 확보되어, 앱이 "다음 배지까지 N"을 배지 9종 전부에 대해 계산할 수 있다. 배지 카탈로그(`GET /routines/badges`)에 진행값을 얹지 않은 이유는 `findCatalog()`가 userId를 받지 않는 순수 마스터 데이터 조회이고, 진행값을 붙이려면 전체 기간 재계산이 매 호출마다 필요하기 때문.
...
- **그룹 무관 내 챌린지 조회**: `GET /routines/challenges/me?status=ONGOING|UPCOMING`은 내가 속한 **모든 그룹**의 챌린지를 마감 임박순(`endDate` 오름차순)으로 반환한다. 그룹별 목록과 동일한 형태에 `groupId`/`groupName`이 추가된다. 그룹별 목록(`findGroupChallenges`)과 달리 `ENDED`는 항상 제외 — 전체 그룹 대상이면 종료 챌린지까지 내리면 페이로드가 커지고, 대시보드 위젯은 "놓치면 안 되는 것"만 보면 되기 때문. `status` 생략 시 `ONGOING`+`UPCOMING` 둘 다. 그룹이 하나도 없으면 빈 배열(200).
- **체크 횟수 집계는 쿼리 1회**: 챌린지마다 기간이 달라 `groupBy`로 나눌 수 없으므로, 참가 중인 습관들의 로그를 전체 기간 한 번에 받아 메모리에서 챌린지별로 센다(`countMyChecks`). 그룹별 목록도 같은 경로를 쓰도록 통합해서, 기존의 "참가 챌린지당 `routineLog.count` 1회" N+1을 함께 제거했다.
- **엔드포인트**: `GET/POST /routines/groups/:groupId/challenges`(목록/생성), `GET /routines/challenges/me`(내 챌린지 전체), `GET/PATCH/DELETE /routines/challenges/:id`(상세/수정/삭제), `POST/DELETE /routines/challenges/:id/join`(참가/취소). `challenges/me`는 라우트 매칭상 반드시 `challenges/:id`보다 먼저 선언되어야 한다.
...
| GET    | `/routines/challenges/me`                | 내가 속한 모든 그룹의 챌린지 (마감 임박순, ENDED 제외) | JWT |
