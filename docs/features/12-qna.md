...
### 답변 작성 (`POST /qna/admin/questions/:questionId/answers`)
- ADMIN 권한 필요
...
### 답변 수정/삭제
- **수정** (`PUT /qna/admin/questions/:questionId/answers/:id`): 내용, 첨부파일 수정
- **삭제** (`DELETE /qna/admin/questions/:questionId/answers/:id`): Soft Delete
...
| POST   | `/qna/admin/questions/:questionId/answers`     | 답변 작성                                 | JWT, Admin         |
| PUT    | `/qna/admin/questions/:questionId/answers/:id` | 답변 수정                                 | JWT, Admin         |
| DELETE | `/qna/admin/questions/:questionId/answers/:id` | 답변 삭제                                 | JWT, Admin         |
...
src/qna/
  dto/
    ...
    attachment.dto.ts
    qna-response.dto.ts       — QuestionDto, PaginatedQuestionDto, AnswerDto, QnaStatisticsDto
  enums/
  ...
  qna.controller.ts           — 사용자 질문 CRUD, resolve
  qna-admin.controller.ts     — ADMIN 답변 CRUD, 통계 조회, 레거시 목록
  qna.service.ts
  qna.scheduler.ts            — 매일 자정 ANSWERED 7일 경과 자동 RESOLVED 처리
  qna.service.spec.ts
  qna.module.ts
```
