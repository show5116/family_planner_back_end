...
**관련 파일**:
- [src/qna/qna.controller.ts](../../src/qna/qna.controller.ts) (예정)
- [src/qna/qna.service.ts](../../src/qna/qna.service.ts) (예정)
...
**관련 파일**:
- [src/qna/qna.service.ts](../../src/qna/qna.service.ts) (예정)
- [src/qna/guards/question-visibility.guard.ts](../../src/qna/guards/question-visibility.guard.ts) (예정)
...
**관련 파일**:
- [src/qna/qna-admin.controller.ts](../../src/qna/qna-admin.controller.ts) (예정)
- [src/qna/qna.service.ts](../../src/qna/qna.service.ts) (예정)
...
- [ ] 질문 작성 + ADMIN에게 알림 발송
- [ ] 질문 공개/비공개 설정
- [ ] 공개 질문 목록 조회 (PUBLIC만 조회됨)
- [ ] 답변 작성 + 상태 변경 (PENDING → ANSWERED) + 사용자에게 알림
- [ ] 질문 해결 완료 (ANSWERED → RESOLVED)
- [ ] 공개 질문은 모든 사용자가 조회 가능
- [ ] 비공개 질문은 본인 또는 ADMIN만 조회 가능
- [ ] ADMIN은 모든 질문 조회 가능
- [ ] 일반 사용자가 답변 작성 시도 → 예외 처리
- [ ] 통계 조회 (카테고리별, 상태별)

### E2E 테스트

- [ ] 사용자가 공개 질문 작성 → ADMIN에게 알림 발송
- [ ] 사용자가 비공개 질문 작성 → 다른 사용자는 조회 불가
- [ ] 공개 질문 목록 조회 → 모든 사용자 접근 가능
- [ ] 비공개 질문 상세 조회 시도 → 본인 아니면 403 Forbidden
- [ ] 공개 질문 상세 조회 → 누구나 조회 가능
- [ ] ADMIN이 답변 작성 → 상태 자동 변경 → 사용자에게 알림 발송
- [ ] 사용자가 해결 완료 처리
- [ ] ADMIN이 전체 질문 목록 조회 (공개/비공개 모두)
...
- [ ] Prisma 스키마 작성 (Question, Answer)
- [ ] Enum 정의 (QuestionCategory, QuestionStatus, QuestionVisibility)
- [ ] QnaModule 생성
- [ ] QnaService 구현
  - [ ] 질문 CRUD (본인 질문만)
  - [ ] 공개 질문 목록 조회 (모든 사용자)
  ...
  - [ ] 통계 조회 (ADMIN 전용)
  - [ ] 공개/비공개 권한 검증
- [ ] QnaController 구현 (사용자용)
- [ ] QnaAdminController 구현 (ADMIN 전용)
- [ ] QuestionVisibilityGuard 구현 (공개/비공개 권한 검증)
- [ ] DTO 작성
- [ ] Swagger 문서화
- [ ] 단위 테스트 작성
- [ ] E2E 테스트 작성
