...
| # | 트리거 | 제목 | 수신자 | 발송 방식 | 파일 |
|---|--------|------|--------|-----------|------|
| 1 | 그룹 가계 내역 등록 | 가계부 내역 등록 | 그룹 멤버 전체 (등록자 제외) | 큐 (즉시) | [household.service.ts](../../src/household/household.service.ts) |
| 2 | 지출 등록 후 월별 예산 초과 | 예산 초과 알림 | 그룹 멤버 전체 | 큐 (즉시) | [household.service.ts](../../src/household/household.service.ts) |
> **본문 예시 (내역 등록)**: `"식비 지출 50,000원이 등록되었습니다"` / `"교통비 입금 10,000원이 등록되었습니다"`
> **본문 예시 (예산 초과)**: `"식비 예산을 초과했습니다. (지출 50,000원 / 예산 40,000원)"`
