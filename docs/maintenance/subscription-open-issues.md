...
| ~~C-1~~ | ~~저장 사용량 분포를 볼 방법이 없다~~ — **✅ 2026-09-12 해결** | 소 | — |
...
**A·C 완료 (2026-09-12).** 남은 B는 해지자가 실제로 생기기 전까지, D는 AI 확장 시점에 한다.
B는 순서가 고정이다 — **B-1(내보내기) 없이 B-2(정리)를 켜면 데이터 파괴**가 된다.
...
### ✅ C-1. 저장 사용량 분포를 볼 방법이 없다 *(2026-09-12 해결)*

**해결** — ADMIN API 두 곳에 얹었습니다.

- `GET /subscription/admin/users` / `users/:userId` — `storageUsedBytes` 추가
- `GET /subscription/admin/storage-stats` — 등급별 요약(합계·중앙값·최대·**한도 임박/초과 인원**)
  + 저장량 구간별 인원

`nearLimitCount`(한도 80% 이상)가 상위 등급 수요 신호입니다. 이 숫자가 늘기 시작하면
[17-subscription.md](../features/17-subscription.md)의 "40GB를 넘는 유저가 생기면"을 실행할 시점입니다.

**함께 정리한 것** — A-3에서 만들었던 집계가 세 번째로 복제될 참이라
[media-usage.util.ts](../../src/common/utils/media-usage.util.ts)로 빼고
만료 알림 스케줄러도 이걸 쓰도록 바꿨습니다. `SubscriptionModule`이 `DiaryModule`을 import하면
순환이라 서비스가 아니라 유틸입니다.

`DiaryMediaQuotaService`는 **일부러 통합하지 않았습니다.** 한도 게이지는 `PENDING` 예약분까지
더해 막아야 하는(돈이 걸린) 판정이고, 여기는 실제 저장된 것만 보는 관측입니다. 질문이 다릅니다.

**남은 것** — 지금은 조회만 있고 추이가 없습니다. "지난달 대비 얼마나 늘었나"를 보려면 스냅샷을
쌓아야 하는데, 판단에 필요해진 뒤에 만들어도 늦지 않습니다.
