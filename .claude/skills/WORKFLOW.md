...
**목적:** curl 등으로 실제 서버를 호출해 응답값을 검증 (validate의 정적 검사와는 별개 축)

**핵심 규칙:** `POST /v1/auth/signup`으로 테스트 유저를 새로 만들지 않는다(실제 메일 발송됨). 항상 기존 테스트 계정(`test-owner@familyplanner.test` / `Test1234!`)을 재사용.

**언제:**
- ✅ 신규/변경 엔드포인트를 실제로 호출해 응답을 확인하고 싶을 때
- ✅ 계획서에 검증 시나리오가 있을 때
