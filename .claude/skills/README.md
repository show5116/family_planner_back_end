...
### 🛡️ safety-check (삭제 영향도 & 다국어 검증)

**목적:** 그룹/유저 삭제 시 데이터 무결성, 스케줄러 타이밍 이슈, 다국어 키 정합성 검증

**포함:**
- 외래키 누락 검사 (groupId/userId → onDelete 설정 확인)
- 스케줄러 타이밍 이슈 (그룹 존재 미확인 후 알림 발송 패턴)
- 다국어 키 누락 및 언어 간 불일치 감지
...
**사용법:** "안전 검사해줘" 또는 `/safety-check`

자세한 내용은 [safety-check.md](safety-check.md)를 참고하세요.
...
### 🧪 api-smoke-test (실제 서버 검증)

**목적:** curl 등으로 서버를 직접 호출해 실제 동작을 검증 (validate의 정적 검사와는 별개)

**포함:**
- 기존 테스트 계정 재사용 절차 (signup API 절대 사용 금지 — 실제 메일 발송됨)
- 서버 기동 확인 → 로그인 → 데이터 준비 → 시나리오 검증 → 정리 순서

**언제:**
- ✅ 신규/변경 엔드포인트의 실제 응답값을 눈으로 확인하고 싶을 때
- ✅ 계획서의 검증 시나리오를 실행할 때

**사용법:** "실제로 호출해서 검증해줘" 또는 `/api-smoke-test`

자세한 내용은 [api-smoke-test.md](api-smoke-test.md)를 참고하세요.
...
### 📰 tech-blog-post (기술 블로그 글 작성)

**목적:** 저장소의 실제 구현 코드를 근거로 Tistory 발행용 SEO 마크다운 글 작성

**포함:**
- 제목 후보 3개 제안 + SEO 키워드
- 서론(페인 포인트 공감) → 본론(문제→원인→해결 코드→왜) → 결론(CTA)
- 이미지 위치/Alt 태그 제안, 해시태그 5~7개
- 시크릿·테스트 계정 마스킹 체크

**언제:** 구현이 끝난 기능이나 트러블슈팅을 글로 남기고 싶을 때

**사용법:** "블로그 글 써줘" 또는 `/tech-blog-post [주제]`

자세한 내용은 [tech-blog-post/SKILL.md](tech-blog-post/SKILL.md)를 참고하세요.
